...
- **URGENT — stolen-vehicle compatibility:** one or more external mods currently interpret V riding the autonomous NCTC service bus as V having stolen that vehicle. Identify which mods/systems are reacting to the Mahir's ownership/theft state and add compatibility so boarding/riding an NCTC service bus can never trigger a stolen-vehicle state, theft penalty, police/wanted response, or any equivalent external-mod consequence for V. Preserve NCTC's autonomous/non-player-vehicle behaviour while applying the narrowest compatibility shim possible.
...
- **Door state loop at service stops:** the passenger doors still sometimes repeatedly close and reopen while the bus is dwelling at a stop. Make the service-stop door state idempotent for the whole dwell, including when V approaches the exit from inside.
- **Progressive braking at normal roadside stops:** when a service stop is directly on the normal traffic lane (no bay spline), the bus should decelerate progressively and naturally instead of braking abruptly at the stop point.
...
- **Save/load service teardown:** make NCTC safe when another save is loaded while a service bus is active. On session/world transition, invalidate the current service generation, cancel/ignore delayed callbacks and AI commands from the previous world, release the controller/bus entity references, and reset runtime service state before another bus can be dispatched. This is intended to prevent stale vehicle/native references from surviving a save change and potentially causing an access-violation crash.
