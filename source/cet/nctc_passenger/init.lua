...
-- Read-only probes. A failed native getter is logged as unavailable, never false.
local function displayRead(label, read)
    local ok, value = pcall(read)
    return label .. "=" .. (ok and tostring(value) or ("UNAVAILABLE(" .. tostring(value) .. ")"))
end

local function traceDisplays(bus, age)
    print("[NCTC Display v4] snapshot age=" .. tostring(age)
        .. " bus=" .. tostring(bus:GetEntityID().hash)
        .. " line=" .. tostring(getFact("nctc_display_line"))
        .. " stop=" .. tostring(getFact("nctc_display_next_stop_id")))
    for _, name in ipairs({"interior_route", "front_line", "front_route", "right_route", "left_route", "rear_line"}) do
        for _, suffix in ipairs({"screen", "display"}) do
            local componentName = "nctc_" .. name .. "_" .. suffix
            local ok, component = pcall(function() return bus:FindComponentByName(CName.new(componentName)) end)
            if not ok or not component then
                print("[NCTC Display v4] " .. componentName .. " missing=" .. tostring(ok) .. " result=" .. tostring(component))
            else
                local parts = {"[NCTC Display v4] " .. componentName}
                for _, field in ipairs({"isEnabled", "localTransform", "parentTransform", "widgetResource", "itemNameToSpawn", "meshTargetBinding", "mesh", "meshAppearance", "visualScale", "tintColor", "spawnDistanceOverride"}) do
                    parts[#parts + 1] = displayRead(field, function() return component[field] end)
                end
                parts[#parts + 1] = displayRead("enabledGetter", function() return component:IsEnabled() end)
                if suffix == "display" then
                    parts[#parts + 1] = displayRead("controllerGetter", function() return component:GetGameController() end)
                    parts[#parts + 1] = displayRead("bindingName", function() return component.meshTargetBinding.bindName end)
                    parts[#parts + 1] = displayRead("bindingEnabled", function() return component.meshTargetBinding.enabled end)
                    local controller = component:GetGameController()
                    if controller then
                        local root = controller:GetRootWidget()
                        parts[#parts + 1] = displayRead("rootSize", function() local s = root:GetSize(); return tostring(s.X) .. "," .. tostring(s.Y) end)
                        parts[#parts + 1] = displayRead("rootOpacity", function() return root:GetOpacity() end)
                        parts[#parts + 1] = displayRead("rootChildren", function() return root:GetNumChildren() end)
                        local childOk, count = pcall(function() return root:GetNumChildren() end)
                        if childOk then
                            for i = 0, math.min(count, 8) - 1 do
                                local child = root:GetWidgetByIndex(i)
                                print("[NCTC Display v12] " .. componentName .. " child=" .. tostring(i)
                                    .. " " .. displayRead("name", function() return child:GetName() end)
                                    .. " " .. displayRead("text", function() return child:GetText() end)
                                    .. " " .. displayRead("size", function() local s = child:GetSize(); return tostring(s.X) .. "," .. tostring(s.Y) end)
                                    .. " " .. displayRead("visible", function() return child:IsVisible() end)
                                    .. " " .. displayRead("opacity", function() return child:GetOpacity() end)
                                    .. " " .. displayRead("fontResource", function() return child.fontFamily end)
                                    .. " " .. displayRead("fontSize", function() return child.fontSize end)
                                    .. " " .. displayRead("tint", function() local t = child:GetTintColor(); return tostring(t.Red) .. "," .. tostring(t.Green) .. "," .. tostring(t.Blue) .. "," .. tostring(t.Alpha) end))
                            end
                        end
                    end
                end
                parts[#parts + 1] = displayRead("localPosition", function() local p = WorldPosition.ToVector4(component.localTransform.Position); return tostring(p) end)
                parts[#parts + 1] = displayRead("localOrientation", function() return tostring(component.localTransform.Orientation) end)
                parts[#parts + 1] = displayRead("parentBind", function() return component.parentTransform.bindName end)
...
registerForEvent("onInit", function()
    print("[NCTC Display v7] SetInteriorUIEnabled caller tracing")
    ObserveBefore("VehicleObject", "SetInteriorUIEnabled", function(vehicle, enabled)
        local ok, panel = pcall(function() return vehicle:FindComponentByName(CName.new("nctc_interior_route_display")) end)
        if ok and panel then
            NCBN.displayInteriorCall = "bus=" .. tostring(vehicle:GetEntityID().hash) .. " requested=" .. tostring(enabled)
            print("[NCTC Display v7] SetInteriorUIEnabled ENTER " .. NCBN.displayInteriorCall)
        end
    end)
    ObserveAfter("VehicleObject", "SetInteriorUIEnabled", function(vehicle, enabled)
        local ok, panel = pcall(function() return vehicle:FindComponentByName(CName.new("nctc_interior_route_display")) end)
        if ok and panel then
            print("[NCTC Display v7] SetInteriorUIEnabled EXIT bus=" .. tostring(vehicle:GetEntityID().hash)
                .. " requested=" .. tostring(enabled) .. " " .. displayRead("panelEnabled", function() return panel:IsEnabled() end))
            NCBN.displayInteriorCall = nil
        end
    end)
    print("[NCTC Display v5] transition diagnostics; no automatic activation")
    -- Observe script-visible calls only. Native internal writes may bypass this hook.
    ObserveBefore("IComponent", "Toggle", function(component, enabled)
        local ok, name = pcall(function() return tostring(component.name) end)
        if ok and string.find(name, "nctc_", 1, true) then
            print("[NCTC Display v5] Toggle requested=" .. tostring(enabled) .. " component=" .. name
                .. " " .. displayRead("before", function() return component:IsEnabled() end)
                .. " SetInteriorUIEnabledContext=" .. tostring(NCBN.displayInteriorCall))
        end
    end)
    ObserveAfter("IComponent", "Toggle", function(component, enabled)
        local ok, name = pcall(function() return tostring(component.name) end)
        if ok and string.find(name, "nctc_", 1, true) then
            print("[NCTC Display v6] Toggle completed requested=" .. tostring(enabled) .. " component=" .. name
                .. " " .. displayRead("after", function() return component:IsEnabled() end))
        end
    end)
    print("[NCTC Display v4] read-only diagnostics loaded; four snapshots per bus")
    ObserveBefore("NCTC.NCTCBusDisplayController", "OnInitialize", function()
        print("[NCTC Display v4] OnInitialize ENTER")
    end)
    ObserveAfter("NCTC.NCTCBusDisplayController", "OnInitialize", function(controller)
        local ok, details = pcall(function()
            local root = controller:GetRootWidget()
            return "root=" .. tostring(root:GetName()) .. " visible=" .. tostring(root:IsVisible())
                .. " line=" .. tostring(getFact("nctc_display_line"))
                .. " stop=" .. tostring(getFact("nctc_display_next_stop_id"))
        end)
        print("[NCTC Display entity-v1] initialized " .. (ok and details or tostring(details)))
    end)
    ObserveAfter("NCTC.NCTCBusDisplayController", "OnRouteChanged", function(controller)
        local ok, details = pcall(function()
            local root = controller:GetRootWidget()
            local labels = {}
            for i = 0, root:GetNumChildren() - 1 do
                local child = root:GetWidgetByIndex(i)
                local isText, value = pcall(function() return child:GetText() end)
                if isText then labels[#labels + 1] = tostring(value) end
            end
            return "line=" .. tostring(getFact("nctc_display_line"))
                .. " stopId=" .. tostring(getFact("nctc_display_next_stop_id"))
                .. " revision=" .. tostring(getFact("nctc_display_revision"))
                .. " text=" .. table.concat(labels, " | ")
        end)
        print("[NCTC Display v16] route refresh " .. (ok and details or tostring(details)))
    end)
...
registerForEvent("onUpdate", function(deltaTime)
    ...
    local player, bus = Game.GetPlayer(), findServiceBus()
    if bus then
        local key = tostring(bus:GetEntityID().hash)
        if NCBN.displayProbeBus ~= key then
            NCBN.displayProbeBus = key
            NCBN.displayProbeAge = 0
            NCBN.displayProbeSample = 1
            NCBN.displayTransitions = {}
            local ok, details = pcall(function()
                return "screen=" .. tostring(bus:FindComponentByName(CName.new("nctc_interior_route_screen")) ~= nil)
                    .. " widget=" .. tostring(bus:FindComponentByName(CName.new("nctc_interior_route_display")) ~= nil)
            end)
            print("[NCTC Display probe-v2] bus=" .. key .. " " .. (ok and details or tostring(details)))
        end
        NCBN.displayProbeAge = (NCBN.displayProbeAge or 0) + (deltaTime or 0)
        if NCBN.displayProbeAge <= 30 then
            for _, name in ipairs({"interior_route", "front_line", "front_route", "right_route", "left_route", "rear_line"}) do
                local ok, state = pcall(function()
                    local widget = bus:FindComponentByName(CName.new("nctc_" .. name .. "_display"))
                    if not widget then return "missing" end
                    return "enabled=" .. tostring(widget:IsEnabled()) .. " controllerPresent=" .. tostring(widget:GetGameController() ~= nil)
                end)
                state = ok and state or ("UNAVAILABLE " .. tostring(state))
                if NCBN.displayTransitions[name] ~= state then
                    print("[NCTC Display v5] state age=" .. tostring(NCBN.displayProbeAge) .. " panel=" .. name
                        .. " previous=" .. tostring(NCBN.displayTransitions[name]) .. " now=" .. state)
                    NCBN.displayTransitions[name] = state
                end
            end
        end
        local deadlines = {0, 1, 3, 10}
        local deadline = deadlines[NCBN.displayProbeSample or 1]
        if deadline and NCBN.displayProbeAge >= deadline then
            local ok, err = pcall(traceDisplays, bus, NCBN.displayProbeAge)
            if not ok then print("[NCTC Display v4] probe error: " .. tostring(err)) end
            NCBN.displayProbeSample = (NCBN.displayProbeSample or 1) + 1
        end
    else
        NCBN.displayProbeBus = nil
    end
