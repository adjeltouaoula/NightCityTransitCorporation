...
local function log_service_loop(quests)
  ...
  local states = {
    ...
    [72] = "route loop: r378c H2 exact-lane departure spline armed",
    [73] = "route loop: r378c H2 exact-lane departure spline active telemetry",
    [74] = "route loop: r378c H2 exact-lane spline -> native traffic handoff",
    [75] = "route loop: r378c H2 exact-lane departure spline FAILED"
  }
  local command_extra = ""
...
local function log_build_revision(quests)
  ...
  if revision == 37803 then
    log("NCTC runtime build=37803 r378c H2 exact lane spline handoff")
