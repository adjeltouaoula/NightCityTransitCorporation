...
local function log_service_loop(quests)
  ...
  local states = {
    ...
    [72] = "route loop: r377c H2 right-lane departure spline armed",
    [73] = "route loop: r377c H2 right-lane departure spline active telemetry",
    [74] = "route loop: r377c H2 right-lane spline -> native traffic handoff",
    [75] = "route loop: r377c H2 right-lane departure spline FAILED"
  }
  local command_extra = ""
...
local function log_build_revision(quests)
  ...
  if revision == 37609 then
    log("NCTC runtime build=37609 r377c H2 right lane deterministic bay")
