...
CLEAN_BRANCH="experiment/r376b-h2-native-spline-departure"
...
grep -q 'nctc_dev_build_revision.*, 37602' source/redscript/NCTC/NCTCTransitSystem.reds
...
grep -q 'r376b H2 native spline arrival + departure' source/cet/nctc_survey/init.lua
test "$(git hash-object source/archive/pc/mod/NCTCH2Spline.archive)" = "$ARRIVAL_BLOB"
test "$(git hash-object source/archive/pc/mod/NCTCH2Spline.archive.xl)" = "$ARRIVAL_XL_BLOB"
test "$(sha256sum source/archive/pc/mod/NCTCDisplayPrototype.archive | awk '{print $1}')" = "$DISPLAY_SHA"
...
git -C "$CLEAN" commit -m "feat: r376b H2 native spline departure"
...
Devkit: r376b H2 native spline arrival + departure
Runtime build: 37602
Branch: $CLEAN_BRANCH
Commit: $CLEAN_SHA
Scope: H2 only. The user-validated r376a arrival archive is byte-identical. r376b adds a separate native worldSplineNode departure spline and rolling handoff back to native traffic.
...
ZIP="$RUNNER_TEMP/NightCityTransitCorporation-0.3.0-devkit-r376b-h2-native-spline-departure-development.zip"
rm -f "$ZIP" "$RUNNER_TEMP/r376b.sha256"
pushd "$ROOT" >/dev/null
zip -qr "$ZIP" .
popd >/dev/null
unzip -t "$ZIP"
sha256sum "$ZIP" | tee "$RUNNER_TEMP/r376b.sha256"

cat > "$RUNNER_TEMP/r376b-metadata.txt" <<EOF
CLEAN_SHA=$CLEAN_SHA
...
cat "$RUNNER_TEMP/r376b-metadata.txt"
