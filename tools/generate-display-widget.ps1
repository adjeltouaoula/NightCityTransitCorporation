...
try {
  ...
  if($result.ToString() -ne 'NoError' -or $readback.RootChunk.LibraryItems.Count -ne 1){throw 'Widget round-trip validation failed'}
  $restored=$readback.RootChunk.LibraryItems[0].PackageData.Data.RootChunk
  if($restored.RootWidget.GetValue().Name.ToString() -ne 'NCTCBusDisplay' -or $restored.GameController.GetValue().ClassName.ToString() -ne 'NCTC.NCTCBusDisplayController'){throw 'Widget controller/root lost during serialization'}
