...
ZIP="$RUNNER_TEMP/NightCityTransitCorporation-0.3.0-devkit-r381c-crowd-empty-path-telemetry-development.zip"
(cd "$ROOT" && zip -qr "$ZIP" .)
unzip -t "$ZIP" >/dev/null
sha256sum "$ZIP" | tee "$RUNNER_TEMP/r381c.sha256"
