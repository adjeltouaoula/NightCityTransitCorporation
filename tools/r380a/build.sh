...
CLEAN_BRANCH="experiment/r380a-h2-vanilla-join-clean"
CLEAN="$RUNNER_TEMP/r380a-clean"
rm -rf "$CLEAN"
...
python3 tools/r380a/patch.py "$CLEAN"
...
grep -Fq 'nctc_dev_build_revision", 38001' "$CLEAN/source/redscript/NCTC/NCTCTransitSystem.reds"
...
git -C "$CLEAN" commit -m "feat: r380a exact vanilla traffic rejoin"
...
ZIP="$RUNNER_TEMP/NightCityTransitCorporation-0.3.0-devkit-r380a-h2-vanilla-join-clean-development.zip"
(cd "$ROOT" && zip -qr "$ZIP" .)
unzip -t "$ZIP" >/dev/null
sha256sum "$ZIP" | tee "$RUNNER_TEMP/r380a.sha256"
printf 'CLEAN_SHA=%s\nARRIVAL_BLOB=%s\nDEPARTURE_BLOB=%s\nDISPLAY_SHA=%s\n' \
  "$CLEAN_SHA" "$ARRIVAL_BEFORE" "$DEPARTURE_BEFORE" "$DISPLAY_BEFORE" > "$RUNNER_TEMP/r380a-metadata.txt"
