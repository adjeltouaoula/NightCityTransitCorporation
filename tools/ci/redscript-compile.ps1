...
if (-not (Test-Path $VanillaBackup)) {
  throw "Vanilla REDscript backup not found: $VanillaBackup. Refusing to compile against a potentially modded final.redscripts bundle."
}
...
Write-Host "REDscript CI game dir: $GameDir"
Write-Host "Staging installed script environment into: $Stage"
...
if ($exitCode -ne 0) {
  throw "scc.exe exited with code $exitCode. See $LogPath"
}
if ($text -match '\[ERROR\s*-|Compilation error|compilation has failed|REDScript compilation has failed') {
  throw "REDscript semantic compilation reported errors. See $LogPath"
}
if (-not (Test-Path $StageBundle)) {
  throw "Compiler produced no final.redscripts output. See $LogPath"
}

Write-Host 'REDscript CI PASSED: NCTC compiled against the installed game bundle and script environment.'

if ($env:GITHUB_ENV) {
  "NCTC_REDSCRIPT_CI_LOG=$LogPath" | Out-File -FilePath $env:GITHUB_ENV -Encoding utf8 -Append
}
