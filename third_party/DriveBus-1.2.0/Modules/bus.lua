...
function Bus:MountPlayer(seat)
    ...
    Cron.After(1.0, function()
        self:NotifyPlayerMountToSystem()
    end)
end

function Bus:NotifyPlayerMountToSystem()
    if self.entity == nil then
        self.log_obj:Record(LogLevel.Error, "NotifyPlayerMountToSystem: entity is nil.")
        return
    end

    local no_driver_event = AIEvent.new()
    no_driver_event.name = CName.new("NoDriver")
    self.entity:QueueEvent(no_driver_event)
    local driver_ready_event = AIEvent.new()
    driver_ready_event.name = CName.new("DriverReady")
    self.entity:QueueEvent(driver_ready_event)
end
